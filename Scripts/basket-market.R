...
transactionData=empresas_enriquecida[!is.na(programas_total) & programas_total!="",c("programas_total")]
...
a<-inspect(association.rules[1:36])
...
names(foo)=c("X1","X2","X3","X4","rhs","support","confidence","coverage","lift","count","cantidad_previos","orden_regla")




bm_empresas=empresas_enriquecida[!is.na(programas_total)]
...
for (i in 1:length(reglas_list)) {
  ...
  if (my_regla$cantidad_previos==1) {
    bm_empresas[grepl( my_regla$rhs, programas_total, fixed = TRUE)==FALSE,regla_bool:=grepl( my_regla$X1, programas_total, fixed = TRUE)]
    ...
    bm_empresas[grepl( my_regla$rhs, programas_total, fixed = TRUE)==FALSE,regla_bool:=ifelse(grepl( my_regla$X1, programas_total, fixed = TRUE) & grepl( my_regla$X2, programas_total, fixed = TRUE),TRUE,FALSE)]
    bm_empresas=bm_empresas[regla_bool==TRUE ,reglas_estrictas:=paste(reglas_estrictas,paste("Regla:",my_regla$orden_regla,sep = " "),sep = "-")] 
    bm_empresas=bm_empresas[regla_bool==TRUE ,productos_ofrecidos_extrictos:=paste(productos_ofrecidos_extrictos,my_regla$rhs,sep = "-")]
  } else if (my_regla$cantidad_previos==3) {
    bm_empresas[grepl( my_regla$rhs, programas_total, fixed = TRUE)==FALSE,regla_bool:=ifelse(grepl( my_regla$X1, programas_total, fixed = TRUE) & grepl( my_regla$X2, programas_total, fixed = TRUE) & grepl( my_regla$X3, programas_total, fixed = TRUE),TRUE,FALSE)]
    bm_empresas=bm_empresas[regla_bool==TRUE ,reglas_estrictas:=paste(reglas_estrictas,paste("Regla:",my_regla$orden_regla,sep = " "),sep = "-")] 
    bm_empresas=bm_empresas[regla_bool==TRUE ,productos_ofrecidos_extrictos:=paste(productos_ofrecidos_extrictos,my_regla$rhs,sep = "-")]
  } else if (my_regla$cantidad_previos==4) {
    bm_empresas[grepl( my_regla$rhs, programas_total, fixed = TRUE)==FALSE,regla_bool:=ifelse(grepl( my_regla$X1, programas_total, fixed = TRUE) & grepl( my_regla$X2, programas_total, fixed = TRUE) & grepl( my_regla$X3, programas_total, fixed = TRUE)  & grepl( my_regla$X4, programas_total, fixed = TRUE),TRUE,FALSE)]
    bm_empresas=bm_empresas[regla_bool==TRUE ,reglas_estrictas:=paste(reglas_estrictas,paste("Regla:",my_regla$orden_regla,sep = " "),sep = "-")] 
    bm_empresas=bm_empresas[regla_bool==TRUE ,productos_ofrecidos_extrictos:=paste(productos_ofrecidos_extrictos,my_regla$rhs,sep = "-")]
  }
